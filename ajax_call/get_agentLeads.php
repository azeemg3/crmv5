...
if($cm->user_access("agentLeads", $_SESSION['sessionId']))
{
    if(isset($_GET['page']))
    {
        $page=$_GET['page'];
    ...
    if(!empty($_POST))
    {
        ...
        // if($spo!='')$whereArr[]="reopen_by=".$spo."";
        if($status!='')$whereArr[]="status='$status'";
        if($lp!='')$whereArr[]="lp='$lp'";
        elseif($leadStatus!="") {$whereArr[]="status='$leadStatus'";}
        $whereArr[]="branch_id=".$_SESSION['branch_id']." AND ".((!empty($spo))?'reopen_by='.$spo.'':'created_by='.$_SESSION['sessionId'].'')."  ";
    ...
    //if($total_rec>10)
    echo '<tr><td colspan="10">'.$cm->pagination($total_rec, $cur_page, $per_page, "get_agentLeads").'</td></tr>';
}
?>
