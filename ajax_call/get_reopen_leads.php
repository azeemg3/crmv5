...
if($cm->user_access("viewAllLeads", $_SESSION['sessionId']))
{
	...
	if(!empty($_POST))
	{
		...
		if($date_frm && $date_to !="")
		$whereArr[]="STR_TO_DATE(reopen_time, '%d-%m-%Y') BETWEEN  STR_TO_DATE('$date_frm', '%d-%m-%Y') AND STR_TO_DATE('$date_to', '%d-%m-%Y ')";
	...
	$result=$cm->selectData("lead", " {$sWhere} AND status!='Trashed' AND reopen_by!=0  ORDER BY id DESC LIMIT $start, $per_page");
	$total_rec=$cm->count_val("lead","id", "{$sWhere} AND status!='Trashed' AND reopen_by!=0");
	$spoName="";
	$count=1;
	$leadId="";
	while($row=$result->fetch_assoc())
	{
		...
		echo '<tr id="'.$row['id'].'">
					...
					<td>'.((!empty($row['travel_datefrm']))?''.$row['travel_datefrm'].'&nbsp;To&nbsp;'.$row['travel_dateto'].'':"N/A").'</td>
					<td>'.$row['services'].'</td>
