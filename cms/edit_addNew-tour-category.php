...
{
	$id=$_GET['cat_id'];
	$result=$cm->selectData("web_tour_cat", "cat_id=".$id."");
    ...
    <div class="row">
      <div class="col-md-12"> 
        ...
        <div class="box box-success">
          ...
          <!-- form start -->
          <form role="form" action="controllers/update_tour_cat" enctype="multipart/form-data" method="post">
            ...
            <div class="box-body">
              ...
              <div class="form-group">
                ...
                <select class="form-control input-ms" name="pkg_id">
                 <?php echo tourSale::tour_list($row['pkg_id']); ?>
                </select>
              </div>
              <div class="form-group">
                <label for="exampleInputEmail1">Thumb Detials</label>
                <input type="text" class="form-control" id="" name="thumb_det" placeholder="Package Name" value="<?php echo $row['thumb_det'] ?>">
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
              <label>Link</label>
               <input name="url_link" type="text" id="hit-url" value="<?php echo $row['url_link'] ?>" class="form-control input-sm">
              </div>
              <div class="form-group">
                <label for="exampleInputFile">Package Header Image</label>
                <input type="file" id="" name="header_img">
                <?php if(!empty($row['header_img'])) { ?>
                <p class="help-block"><img src="cms/../<?php echo $row['header_img'] ?>" width="50" height="50" /></p>
                <?php } ?>
              </div>
          ...
          <table class="table table-bordered table-striped">
           ...
           <tbody id="get_session_content">
           <?php
		   $count=1;
		   $cd_result=$cm->selectData("web_tour_cat_det", "cat_id=".$row['cat_id']."");
		   while($cdRow=$cd_result->fetch_array())
		   {
			   $thisData[]=array('content_heading'=>$cdRow['heading_text'], 'content_img'=>preg_replace("/[^.a-zA-Z]+/", "-", 
			   ((!empty($cdRow['content_img']))?''.$_POST['content_img'].'':''.$cdRow['det_img'].'')), 
		'content_text'=>$cdRow['text_det'], 'uid'=>$cdRow['id']);
			$_SESSION['content_rec']=$thisData;
			echo '<tr id="'.$cdRow['id'].'">
				<td>'.$count++.'</td>
				<td>'.$cdRow['heading_text'].'</td>
				<td>'.$cdRow['text_det'].'</td>
				<td><img src="tour-packages-images/'.$cdRow['det_img'].'" width="50"></td>
				<td><a href="javascript::void()" onclick="more_content(\''.$cdRow['id'].'\')"><i class="fa fa-edit"></i></a> 
			<a href="javascript::void(0)" onClick="del_tour_cat(\''.$cdRow['id'].'\')"><i class="fa fa-trash"></i></a></td>
			</tr>
			';
		   }
		    ?>
          ...
          <div class="box-footer">
              ...
              <?php if(empty($row['cat_id'])) echo 'Submit'; else echo 'Update'; ?>
              </button>
            </div>
          </form>
        </div>
