...
$msg="";
if(isset($_POST['approved']) && md5($_POST['password'])==$cm->password_app($_SESSION['sessionId']) && !empty($_POST['bank_id']))
{
	$rec_id=$_POST['rec_id'];
	$ref_id=$_POST['ref_id'];
	$TPRV=$_POST['TPRV'];
	...
	if(!empty($rec_id))
	{
		...
		{
			$cm->update("payment_reciept", "TPRV=".$TPRV.", status='approved', trans_acc_id='$bank', app_date='".$cm->today()."', 
			app_by=".$_SESSION['sessionId']."", 
			"id=".$rec_id."");
			$cm->update("trans","status='approved', trans_acc_id='$bank'","trans_code=".$transCode."");
	...
	else if(isset($_POST['ref_id']) && !empty($_POST['ref_id']))
	{
		...
		$cm->update("refund_payment", "status='approved', app_by='".$_SESSION['sessionId']."', app_date='".$cm->today()."'", "id=".$ref_id."");
		/* Approved the Transaction Account  */
...
<section class="content-header" style="border-bottom:1px solid;padding-bottom: 14px;">
	...
	<h2 style="text-align:center;display:block;margin:0px;padding:10px 0px;font-style:italic;background:#cdcccc;"><span class="main-heading">Pending Vouchers</span></h2>
					...
					while($row=$query->fetch_assoc())
					{
						echo'
							<tr>
								...
								<td>'.$row['amount'].'</td>
								<td>'.$row['status'].'</td>
								<td><a href="cheque_det?rec_id='.$row['id'].'">View Receipt</a></td>
							</tr>
						';
					}
					while($row=$query_r->fetch_assoc())
					{
						echo'
							<tr>
								...
								<td>'.$row['amount'].'</td>
								<td>'.$row['status'].'</td>
								<td><a href=receipt_det?ref_id='.$row['id'].'>View Receipt</a></td>
							</tr>
						';
					}
				?>
