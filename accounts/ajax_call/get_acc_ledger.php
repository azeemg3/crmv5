...
if(isset($_POST['trans_acc_id']) && !empty($_POST['trans_acc_id']))
{
	...
	if(!empty($_POST['dt_frm']) && !empty($_POST['dt_to']))
	{
		$dt_frm=$_POST['dt_frm'];
		$dt_to=$_POST['dt_to'];
		$whereArray[]="STR_TO_DATE(trans_date, '%d-%m-%Y') BETWEEN  STR_TO_DATE('$dt_frm', '%d-%m-%Y') AND STR_TO_DATE('$dt_to', '%d-%m-%Y ') AND trans.status='approved' AND trans_acc_id=".$trans_acc_id.""; 
